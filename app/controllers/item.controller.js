...
exports.get = (req,res)=>{
    if(req.query.uid && req.query.id) Item.findById(req.query.id, (err,data)=>sendData(err,data,req,res));
        ...
        const query = { uid: req.query.uid, $or: [{ name: { $regex: req.query.query, $options: 'i' }}, { subcategory: { $regex: req.query.query, $options: 'i' }}, { category: { $regex: req.query.query, $options: 'i' }}] };
